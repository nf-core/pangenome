...
```bash
nextflow run nf-core/pangenome --input samplesheet.csv -profile docker
```
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/pangenome`](https://hub.docker.com/r/nfcore/pangenome/)
* `singularity`
...
You can find the keys to specify the genomes in the [iGenomes config file](https://github.com/nf-core/pangenome/blob/master/conf/igenomes.config). Common genomes that are supported are:
