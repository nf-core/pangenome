...
workflow ODGI_QC {
    ...
    if (community_mode) {
        ...
        ch_graph_qc = ODGI_STATS.out.yaml.collect().map{meta_s, s, meta_g, g -> [ meta_g , s, g ]}.view()
        // ch_graph_qc.view()
        // ch_graph_qc = ODGI_STATS.out.yaml.map{meta, yaml -> [ [ id: meta.id.replaceFirst(".seqwish",".gfaffix")], yaml ]}.view()
